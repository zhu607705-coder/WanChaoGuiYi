    ...
    public sealed class DomainEconomySystem : IGameSystem
    {
        ...
        public void ExecuteTurn(GameContext context)
        {
            ...
            for (int i = 0; i < context.State.factions.Count; i++)
            {
                ...
                if (faction.food < 0)
                {
                    ApplyGrainShortagePressure(context, faction);
                }
        ...
        private void ApplyGrainShortagePressure(GameContext context, FactionState faction)
        {
            if (context == null || context.State == null || faction == null) return;

            for (int i = 0; i < faction.regionIds.Count; i++)
            {
                RegionState region = context.State.FindRegion(faction.regionIds[i]);
                StrategyCausalRules.ApplyGrainShortagePressure(faction, region, null);
            }
            ...
            if (worldState != null && worldState.Map != null)
            {
                ...
                {
                    if (army == null || army.ownerFactionId != faction.id) continue;
                    StrategyCausalRules.ApplyGrainShortagePressure(faction, null, army);
                }
            }

            context.State.AddLog("economy", faction.name + " grain shortage pressure: supply/order/acceptance decreased. Source: " + StrategyMapRulebook.FoodSource);
        }
