...
namespace WanChaoGuiYi
{
    public sealed class BattleResolver : MonoBehaviour
    {
        ...
        public BattleResult Resolve(GameContext context, ArmyState attacker, ArmyState defender)
        {
            ...
            int attackerSoldiersBefore = attacker != null ? attacker.soldiers : 0;
            int defenderSoldiersBefore = defender != null ? defender.soldiers : 0;
            ...
            BattleResult result = new BattleResult
            {
                ...
                battleRegionId = defender != null ? defender.regionId : null,
                attackerPower = attackerPower,
                defenderPower = defenderPower,
                attackerSupplyPowerPercent = 100,
                defenderSupplyPowerPercent = 100,
                attackerLowestSupply = -1,
                defenderLowestSupply = -1,
            ...
            result.attackerSoldiersAfter = attacker != null ? attacker.soldiers : 0;
            result.defenderSoldiersAfter = defender != null ? defender.soldiers : 0;
        ...
        public BattleResult ResolveSemiAuto(GameContext context, BattleSetup setup, List<BattleLog> battleLog)
        {
            ...
            bool attackerWon = attackerPower >= defenderPower;
            int attackerSoldiersBefore = SumDeploymentSoldiers(setup.attackerDeployments);
            int defenderSoldiersBefore = SumDeploymentSoldiers(setup.defenderDeployments);

            BattleResult result = new BattleResult
            {
                attackerArmyId = setup.attackerFactionId,
                defenderArmyId = setup.defenderFactionId,
                battleRegionId = setup.terrainId,
                attackerPower = attackerPower,
                defenderPower = defenderPower,
                attackerSupplyPowerPercent = 100,
                defenderSupplyPowerPercent = 100,
            ...
            result.attackerSoldiersAfter = SumFactionDeploymentSoldiers(context, setup.attackerDeployments);
            result.defenderSoldiersAfter = SumFactionDeploymentSoldiers(context, setup.defenderDeployments);
        ...
        private static int SumDeploymentSoldiers(BattleDeployment[] deployments)
        {
            int total = 0;
            if (deployments == null) return total;

            for (int i = 0; i < deployments.Length; i++)
            {
                if (deployments[i] != null)
                {
                    total += deployments[i].soldierCount;
                }
            }
        ...
        private static int SumFactionDeploymentSoldiers(GameContext context, BattleDeployment[] deployments)
        {
            int total = 0;
            if (context == null || context.State == null || deployments == null) return total;

            for (int i = 0; i < deployments.Length; i++)
            {
                if (deployments[i] == null) continue;
                ArmyState army = FindArmy(context, deployments[i].armyId);
                if (army != null)
                {
                    total += army.soldiers;
