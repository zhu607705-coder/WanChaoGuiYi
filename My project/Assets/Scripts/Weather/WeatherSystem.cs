...
namespace WanChaoGuiYi
{
    public sealed class WeatherSystem : MonoBehaviour, IGameSystem
    {
        ...
        {
            WeatherEffect effect;
            string weatherId = context != null && context.State != null ? context.State.currentWeatherId : null;
            if (!string.IsNullOrEmpty(weatherId) && WeatherEffects.TryGetValue(weatherId, out effect))
            {
                return effect;
            }
