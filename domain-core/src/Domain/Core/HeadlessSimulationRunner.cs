...
namespace WanChaoGuiYi
{
    ...
    public sealed class HeadlessSimulationRunner
    {
        ...
        public HeadlessSimulationResult RunFrontlineLogisticsScheduleAndOccupationQueue(IDataRepository data, string playerFactionId)
        {
            ...
            runtime.movementSystem.ExecuteTurn(runtime.context);
            int foodAfterFirstLogistics = faction.food;
            runtime.warResolutionSystem.ExecuteTurn(runtime.context);
            RunEconomyWithSnapshot(runtime);
            result.turnsExecuted = 1;
            AddAssertion(result, "logistics.first_turn_builds_supply_node_and_moves_segment", "frontline",
                source.supplyNode &&
                    ...
                    foodAfterFirstLogistics < foodBeforePlan,
                "first logistics turn builds a supply node and spends grain",
                "sourceNode=" + source.supplyNode + " runtimeNode=" + runtimeSource.supplyNode + " completed=" + army.frontlineLogisticsCompletedSegments + " logisticsFood=" + foodAfterFirstLogistics + " turnFood=" + faction.food,
        ...
        private static int CalculateExpectedArmyUpkeep(GameContext context, FactionState faction, NumericContext numericContext, bool food)
        {
            ...
            for (int i = 0; i < context.State.armies.Count; i++)
            {
                ...
                upkeep += CalculateExpectedRawArmyUpkeep(army, unit, food);
            }

            return upkeep;
        }

        private static int CalculateExpectedRawArmyUpkeep(ArmyState army, UnitDefinition unit, bool food)
        {
