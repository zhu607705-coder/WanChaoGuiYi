...
namespace WanChaoGuiYi
{
    public sealed class MapCommandService
    {
        ...
        public bool RetreatArmy(string armyId, string targetRegionId)
        {
            ...
            if (!IsValidRetreatTarget(army, targetRegionId))
            {
                AddLog("war", army.id + "撤退失败。原因：目标不是相邻己方地区。影响：军队仍留在当前接敌。");
                return false;
            }
        ...
        public bool IsRouteStepValid(string fromRegionId, string nextRegionId)
        {
            return queries != null && queries.AreNeighbors(fromRegionId, nextRegionId);
        }
        ...
        private bool IsValidRetreatTarget(ArmyRuntimeState army, string targetRegionId)
        {
