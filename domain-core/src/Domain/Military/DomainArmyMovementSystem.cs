    ...
    public sealed class DomainArmyMovementSystem : IGameSystem
    {
        ...
        private void AdvanceMapLedArmies(GameContext context, MapState mapState)
        {
            ...
            foreach (ArmyRuntimeState army in mapState.ArmiesById.Values)
            {
                ...
                if (!CanAdvanceRouteStep(army, fromRegionId, nextRegionId))
                {
                    context.State.AddLog("war", army.id + "行军中止。原因：路线与当前位置或邻接关系不一致。影响：系统清空旧命令以避免跨区跳跃。");
                    StopInvalidRoute(army);
                    continue;
                }

                mapState.MoveArmyToRegion(army.id, nextRegionId);
                SyncLegacyArmyRegion(context, army.id, nextRegionId);
                StrategyCausalRules.ApplyArmyMovementSupplyCost(army);
                ...
                context.State.AddLog("war", army.id + "行军消耗军需，补给降至" + army.supply + "。原因：长距离推进会消耗粮秣。影响：若补给不足，后续作战与撤退承压。");
        ...
        private bool CanAdvanceRouteStep(ArmyRuntimeState army, string fromRegionId, string nextRegionId)
        {
            if (army == null || army.route == null || army.route.Count < 2) return false;
            if (army.route[0] != fromRegionId) return false;
            return mapCommandService == null || mapCommandService.IsRouteStepValid(fromRegionId, nextRegionId);
        }

        private static void StopInvalidRoute(ArmyRuntimeState army)
        {
            if (army == null) return;
            army.targetRegionId = null;
            if (army.route != null) army.route.Clear();
            army.task = ArmyTask.Idle;
        }
